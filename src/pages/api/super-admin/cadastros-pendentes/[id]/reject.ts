...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return res.status(200).json({ success: true, message: 'Cadastro rejeitado (tenant suspensa)' })
  } catch (err: unknown) {
    console.error('super-admin reject:', err)
    return res.status(500).json({
      error: err instanceof Error ? err.message : 'Erro ao rejeitar cadastro',
    })
  }
}
