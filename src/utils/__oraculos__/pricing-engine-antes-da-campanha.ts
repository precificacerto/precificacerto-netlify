/**
 * ORÁCULO CONGELADO — o `pricing-engine.ts` COMO ERA ANTES DA CAMPANHA.
 *
 * Cópia literal de `src/utils/pricing-engine.ts` no commit `423b1d4`, o último antes de
 * `2704d61` ("o divisor passa a aceitar o coeficiente da operação por fora"), que abriu a
 * série de mudanças do motor.
 *
 * >>> NÃO EDITAR, NUNCA <<<
 * Este arquivo não é código do produto: é a MEDIDA contra a qual a regressão é conferida.
 * Corrigir um defeito aqui, melhorar um nome ou "sincronizar" com o motor atual destrói
 * exatamente o que ele serve para provar. Se o motor de hoje diverge dele num caso, a
 * pergunta é sobre o motor de hoje.
 *
 * Por que um arquivo e não um conjunto de números esperados: números copiados para dentro do
 * teste são escritos POR QUEM ACABOU DE FAZER A MUDANÇA, e é a pessoa com menos condições de
